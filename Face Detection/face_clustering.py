...
#   httpsgithub.comdaviskingdlibblobmasterexamplesdnn_metric_learning_on_images_ex.cpp
...
if len(sys.argv) != 5
    print(
        Call this program like thisn
           .face_clustering.py shape_predictor_5_face_landmarks.dat dlib_face_recognition_resnet_model_v1.dat ..examplesfaces output_foldern
        You can download a trained facial shape predictor and recognition model fromn
            httpdlib.netfilesshape_predictor_5_face_landmarks.dat.bz2n
            httpdlib.netfilesdlib_face_recognition_resnet_model_v1.dat.bz2)
    exit()
...
for f in glob.glob(os.path.join(faces_folder_path, .jpg))
    ...
    for k, d in enumerate(dets)
        # Get the landmarksparts for the face in box d.
        shape = sp(img, d)
